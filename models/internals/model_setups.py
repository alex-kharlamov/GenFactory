...
class Config:
    ...
    def mlp_class(self) -> Type:
        import models.internals.lit_gpt as lit_gpt

        # `self._mlp_class` cannot be the type to keep the config json serializable
        return getattr(lit_gpt, self._mlp_class)

    @property
    def norm_class(self) -> Type:
        # `self._norm_class` cannot be the type to keep the config json serializable
        if self._norm_class == "RMSNorm":
            from models.internals.rmsnorm import RMSNorm

            return RMSNorm
        elif self._norm_class == "FusedRMSNorm":
            from models.internals.rmsnorm import FusedRMSNorm
            return FusedRMSNorm
